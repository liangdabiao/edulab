...
def solve_cube_direction_cosines(edge=1, scale=2):
    """正方体中方向余弦与空间角的正弦、余弦。

    以向量 A₁C 为例，计算方向余弦、方向正弦、线面角和异面直线夹角。
    """
    pts = cube(edge)
    v = pts["C"] - pts["A1"]  # A₁C
    norm_v = sp.sqrt(sum(c**2 for c in v))

    cos_alpha = sp.simplify(v[0] / norm_v)
    cos_beta = sp.simplify(v[1] / norm_v)
    cos_gamma = sp.simplify(v[2] / norm_v)
    cos2_sum = sp.simplify(cos_alpha**2 + cos_beta**2 + cos_gamma**2)

    sin_alpha = sp.sqrt(1 - cos_alpha**2)
    sin_beta = sp.sqrt(1 - cos_beta**2)
    sin_gamma = sp.sqrt(1 - cos_gamma**2)
    sin2_sum = sp.simplify(sin_alpha**2 + sin_beta**2 + sin_gamma**2)

    n_base = normal_from_points(pts["A"], pts["B"], pts["D"])
    sin_lp = line_plane_angle_sin(v, n_base)
    cos_ll = line_line_angle_cos(v, pts["B"] - pts["A"])

    return {
        "answer_latex": tex(sin_lp),
        "math_points": {k: tex_vec(v) for k, v in pts.items()},
        "three_points": to_three(pts, scale=scale),
        "vals": {
            "v": tex_vec(v),
            "norm_v": tex(sp.simplify(norm_v)),
            "cos_alpha": tex(cos_alpha),
            "cos_beta": tex(cos_beta),
            "cos_gamma": tex(cos_gamma),
            "cos2_sum": tex(cos2_sum),
            "sin_alpha": tex(sin_alpha),
            "sin_beta": tex(sin_beta),
            "sin_gamma": tex(sin_gamma),
            "sin2_sum": tex(sin2_sum),
            "sin_lp": tex(sin_lp),
            "cos_ll": tex(cos_ll),
        },
        "_exact": {"cos2_sum": cos2_sum, "sin2_sum": sin2_sum, "sin_lp": sin_lp, "cos_ll": cos_ll},
    }
